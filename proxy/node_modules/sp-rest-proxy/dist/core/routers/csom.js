...
var CsomRouter = (function (_super) {
    ...
    function CsomRouter(context, settings) {
        ...
        _this.router = function (req, res, _next) {
            ...
            req.on('end', function () {
                Promise.all([
                    _this.spr.requestDigest((endpointUrl).split('/_vti_bin')[0]),
                    _this.util.getAuthOptions()
                ])
                    .then(function (_a) {
                    var digest = _a[0], opt = _a[1];
                    var headers = __assign({}, opt.headers, { 'Accept': '*/*', 'Content-Type': 'text/xml', 'X-Requested-With': 'XMLHttpRequest', 'X-RequestDigest': digest });
